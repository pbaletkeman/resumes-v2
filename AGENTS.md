...
| Markdown lint | `npx --yes markdownlint-cli2 "*.md" "docs/*.md" "ui/README.md" "scratch/*.md"` (config: `.markdownlint-cli2.jsonc`) |
| Markdown lint (auto-fix) | `npx --yes markdownlint-cli2 --fix "*.md" "docs/*.md" "ui/README.md" "scratch/*.md"` |
...
tests/
  ...
  test_cover_letter_validation.py  # Cover Letter post-validation tests (117 tests)
  ...
  test_skill_normalizer.py         # SkillNormalizer canonical taxonomy tests (22 tests)
  ...
  test_pipeline.py                 # AgentRunner / run_resume_pipeline orchestration + template passthrough + CLI/web persisted-override alignment (27 tests, stub agents)
  ...
  test_web_pipeline.py             # Web sync + async pipeline routes + resume template validation + CLI/web entry-point parity (16 tests)
...
app/                       # FastAPI web API layer
  ...
  schemas.py               # Pydantic request/response models (PipelineRun*, Task*, File*, Model*)
...
- **Model overrides** via env vars: `COVER_LETTER_AGENT_MODEL=gpt-4o`, `COVER_LETTER_AGENT_PROVIDER=openai`. Prefix is the uppercased agent name. The web API also persists per-agent overrides in SQLite (`app/model_store.py`); `create_runner_from_config()` loads those by default so the CLI, `sample_run()`, and the web API resolve the identical effective model configuration (pass `overrides={}` to force environment-only).
