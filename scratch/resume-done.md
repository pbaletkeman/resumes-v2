...
- [Resume Pipeline — Completed Work](#resume-pipeline--completed-work)
  - [Overview](#overview)
  - [Current Architecture](#current-architecture)
    - [What exists now](#what-exists-now)
  - [Phase 1: Core Infrastructure](#phase-1-core-infrastructure)
    - [1.1 Refactor `client/model_client.py`](#11-refactor-clientmodel_clientpy)
    - [1.2 Add error handling to LLM clients](#12-add-error-handling-to-llm-clients)
    - [1.3 Clean up `requirements.txt`](#13-clean-up-requirementstxt)
  - [Phase 2: Document Parsing (Agents 1 \& 2)](#phase-2-document-parsing-agents-1--2)
    - [2.1 Expand `client/format_detector.py`](#21-expand-clientformat_detectorpy)
    - [2.2 Agent Infrastructure](#22-agent-infrastructure)
    - [2.3 JD Parsing Agent](#23-jd-parsing-agent)
    - [2.4 Resume Parsing Agent](#24-resume-parsing-agent)
  - [Phase 3: Analysis \& Rewriting (Agents 3-5)](#phase-3-analysis--rewriting-agents-3-5)
    - [3.1 Create Gap Analysis Agent (`client/agents/gap_analysis.py`)](#31-create-gap-analysis-agent-clientagentsgap_analysispy)
    - [3.2 Create Resume Rewrite Agent (`client/agents/resume_rewrite.py`)](#32-create-resume-rewrite-agent-clientagentsresume_rewritepy)
    - [3.3 Create ATS Compliance Agent (`client/agents/ats_compliance.py`)](#33-create-ats-compliance-agent-clientagentsats_compliancepy)
  - [Phase 4: Polish \& Cover Letter (Agents 6-7)](#phase-4-polish--cover-letter-agents-6-7)
    - [4.1 Create Tone Polishing Agent (`client/agents/tone_polishing.py`)](#41-create-tone-polishing-agent-clientagentstone_polishingpy)
    - [4.2 Create Cover Letter Agent (`client/agents/cover_letter.py`)](#42-create-cover-letter-agent-clientagentscover_letterpy)
    - [4.3 Fix LLM Fallback Falsehoods — completed items](#43-fix-llm-fallback-falsehoods--completed-items)
      - [A. Improve Post-Validation for Resume Rewrite (HIGH priority) — ✅ DONE](#a-improve-post-validation-for-resume-rewrite-high-priority---done)
      - [B. Improve Post-Validation for Cover Letter (HIGH priority) — ✅ DONE](#b-improve-post-validation-for-cover-letter-high-priority---done)
      - [C. Improve Fallback Templates (MEDIUM priority) — ✅ DONE](#c-improve-fallback-templates-medium-priority---done)
      - [D. Add Fallback Detection Logging (LOW priority) — ✅ DONE](#d-add-fallback-detection-logging-low-priority---done)
      - [E. Strengthen Prompts (root-cause mitigation, pairs with A/B) — ✅ DONE](#e-strengthen-prompts-root-cause-mitigation-pairs-with-ab---done)
      - [F. Add `company_name` to `JDParsingOutput` and `company_signals` (HIGH priority, prerequisite for B.2/C.2) — ✅ DONE](#f-add-company_name-to-jdparsingoutput-and-company_signals-high-priority-prerequisite-for-b2c2---done)
  - [Phase 5: Agent Orchestration](#phase-5-agent-orchestration)
    - [5.1 Implement `AgentRunner` in `pipeline.py`](#51-implement-agentrunner-in-pipelinepy)
    - [5.2 Wire up the 7-agent pipeline](#52-wire-up-the-7-agent-pipeline)
  - [Phase 6: Output \& Validation](#phase-6-output--validation)
    - [6.1 Add Pydantic models (`client/models.py`)](#61-add-pydantic-models-clientmodelspy)
    - [6.2 Output Formatting Helpers (Phase 6.A)](#62-output-formatting-helpers-phase-6a)
      - [6.A.1 `format_resume_markdown()` (`client/formatter.py`)](#6a1-format_resume_markdown-clientformatterpy)
      - [6.A.2 `format_resume_plain()` (`client/formatter.py`)](#6a2-format_resume_plain-clientformatterpy)
      - [6.A.3 `format_cover_letter()` (`client/formatter.py`)](#6a3-format_cover_letter-clientformatterpy)
      - [6.A.4 Unit tests for formatting helpers](#6a4-unit-tests-for-formatting-helpers)
    - [6.3 Template-Based Multi-Format Renderer (Phase 6.B)](#63-template-based-multi-format-renderer-phase-6b)
      - [6.B.1 `ResumeRenderer` class skeleton (`client/templates/renderer.py`)](#6b1-resumerenderer-class-skeleton-clienttemplatesrendererpy)
      - [6.B.2 `render_markdown()` on `ResumeRenderer`](#6b2-render_markdown-on-resumerenderer)
      - [6.B.3 Add `render_cover_letter_plaintext()` and `render_cover_letter_markdown()`](#6b3-add-render_cover_letter_plaintext-and-render_cover_letter_markdown)
      - [6.B.4 Add `build_output_path()` static method](#6b4-add-build_output_path-static-method)
      - [6.B.5 Add DOCX generation (`render_docx()`)](#6b5-add-docx-generation-render_docx)
      - [6.B.6 Add PDF generation (`render_pdf()`)](#6b6-add-pdf-generation-render_pdf)
      - [6.B.7 Add `render_all()` convenience method](#6b7-add-render_all-convenience-method)
      - [6.B.8 Wire renderer into pipeline (`pipeline.py`)](#6b8-wire-renderer-into-pipeline-pipelinepy)
      - [6.B.9 Add unit tests for `ResumeRenderer`](#6b9-add-unit-tests-for-resumerenderer)
    - [6.10 Dependency Library Notes \& Cautions](#610-dependency-library-notes--cautions)
  - [Phase 8: Enforce Structured JSON Output via `response_format`](#phase-8-enforce-structured-json-output-via-response_format)
    - [8.1 Extend the `ModelClient` interface](#81-extend-the-modelclient-interface)
    - [8.2 OllamaClient — pass `format="json"` / schema](#82-ollamaclient--pass-formatjson--schema)
    - [8.3 OpenAIClient — pass `response_format={"type": "json_object"}` / schema envelope](#83-openaiclient--pass-response_formattype-json_object--schema-envelope)
    - [8.4 Update every call site](#84-update-every-call-site)
    - [8.5 Keep the `_parse_json` safety net](#85-keep-the-_parse_json-safety-net)
    - [8.6 Tests](#86-tests)
    - [8.7 ✅ DONE — provider-native JSON Schema (Structured Outputs)](#87--done--provider-native-json-schema-structured-outputs)
    - [8.8 ✅ DONE — shared JSON parser](#88--done--shared-json-parser)
  - [Phase 8 (Contact Info): Contact Information Extraction \& Cover Letter Integration](#phase-8-contact-info-contact-information-extraction--cover-letter-integration)
    - [8.1 Add contact fields to `ResumeParsingOutput`](#81-add-contact-fields-to-resumeparsingoutput)
    - [8.2 Update cover letter templates to include contact info](#82-update-cover-letter-templates-to-include-contact-info)
    - [8.3 Pass contact info to the Cover Letter agent and use it in generation](#83-pass-contact-info-to-the-cover-letter-agent-and-use-it-in-generation)
  - [File Structure (Current State)](#file-structure-current-state)
  - [Phase 8.5: Skill Normalization \& Canonical Taxonomy](#phase-85-skill-normalization--canonical-taxonomy)
    - [8.5.1 Create skill normalization module](#851-create-skill-normalization-module)
    - [8.5.2 Integrate skill normalization into JD Parsing Agent](#852-integrate-skill-normalization-into-jd-parsing-agent)
    - [8.5.3 Integrate skill normalization into Resume Parsing Agent](#853-integrate-skill-normalization-into-resume-parsing-agent)
    - [8.5.4 Integrate skill normalization into Gap Analysis Agent](#854-integrate-skill-normalization-into-gap-analysis-agent)
    - [8.5.5 Integrate skill normalization into Resume Rewrite Agent](#855-integrate-skill-normalization-into-resume-rewrite-agent)
    - [8.5.6 Integrate skill normalization into Cover Letter Agent](#856-integrate-skill-normalization-into-cover-letter-agent)
  - [Phase 9: Cover Letter Creation Fixes (a/b/c)](#phase-9-cover-letter-creation-fixes-abc)
    - [9.1 Chronological ordering of experience (in `resume_rewrite.py`)](#91-chronological-ordering-of-experience-in-resume_rewritepy)
    - [9.2 Company Name must come from the JD, not the candidate's resume](#92-company-name-must-come-from-the-jd-not-the-candidates-resume)
    - [9.3 Candidate Name must come from the candidate's resume](#93-candidate-name-must-come-from-the-candidates-resume)
    - [9.4 Cross-cutting notes](#94-cross-cutting-notes)
  - [Tooling](#tooling)
  - [Phase 10: FastAPI Web API Layer (`app/`)](#phase-10-fastapi-web-api-layer-app)
    - [10.1 Why `_run_pipeline_core()` instead of `run_resume_pipeline()`](#101-why-_run_pipeline_core-instead-of-run_resume_pipeline)
    - [10.2 Files created](#102-files-created)
    - [10.3 Routes](#103-routes)
    - [10.4 Input handling](#104-input-handling)
    - [10.5 File persistence \& safety](#105-file-persistence--safety)
  - [Verification Bugs (fixed 2026-08-06 during `resume-verify.md` live runs)](#verification-bugs-fixed-2026-08-06-during-resume-verifymd-live-runs)
    - [V1 — Event-loop lifecycle: `RuntimeError: Event loop is closed` — ✅ FIXED](#v1--event-loop-lifecycle-runtimeerror-event-loop-is-closed---fixed)
    - [V2 — `_extract_field` returns whole model instead of named field — ✅ FIXED](#v2--_extract_field-returns-whole-model-instead-of-named-field---fixed)
  - [Phase 7: Testing \& Docs — ALL COMPLETE (2026-08-08)](#phase-7-testing--docs--all-complete-2026-08-08)
    - [7.1 Live end-to-end integration test — `test_real_files.py` — ✅ DONE (2026-08-07)](#71-live-end-to-end-integration-test--test_real_filespy---done-2026-08-07)
    - [7.2 Add unit tests (`tests/`) — per-agent + pipeline coverage — ✅ DONE (2026-08-07)](#72-add-unit-tests-tests--per-agent--pipeline-coverage---done-2026-08-07)
      - [7.2.1 Agent contract tests (55 tests, `FakeClient` in `conftest.py`)](#721-agent-contract-tests-55-tests-fakeclient-in-conftestpy)
      - [7.2.2 `tests/test_pipeline.py` — pipeline wiring with mocked agents — ✅ DONE (2026-08-07; 17 tests)](#722-teststest_pipelinepy--pipeline-wiring-with-mocked-agents---done-2026-08-07-17-tests)
    - [7.3 Populate `docs/` — ✅ DONE (2026-08-08)](#73-populate-docs---done-2026-08-08)
    - [7.4 Web API tests (`tests/test_web_*.py`) — ✅ DONE (2026-08-08; 43 tests)](#74-web-api-tests-teststest_web_py---done-2026-08-08-43-tests)
